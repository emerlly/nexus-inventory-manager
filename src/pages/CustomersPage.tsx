...
export default function CustomersPage() {
  ...
  const save = useMutation({
    mutationFn: (d: CustomerFormData) => editing ? customerService.update(editing._id, d) : customerService.create(d),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["customers"] }); setOpen(false); toast({ title: "Salvo!" }); },
    onError: () => toast({ variant: "destructive", title: "Erro ao salvar" }),
  });

  const del = useMutation({
    mutationFn: (c: Customer) => customerService.remove(c._id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["customers"] }); toast({ title: "Excluído!" }); },
  });
